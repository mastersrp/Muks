...
if ((isset($_GET['type']) && $_GET['type'] == "A") && (isset($_GET['item_id']) && isnum($_GET['item_id']))) {
	...
	$result = dbquery(
		"SELECT fp.post_message, fp.post_datestamp, fp.post_edittime, fp.post_author, fp.post_edituser, fp.post_alias, fp.post_replynum,
	...
	$result = dbquery(
		"SELECT fp.post_message, fp.post_datestamp, fp.post_edittime, fp.post_author as post_author, fp.post_edituser, fp.post_alias, fp.post_replynum,
		fu.user_name AS user_name, fu.user_status AS user_status, fu.user_aliases,
		ft.thread_subject, ff.forum_access
		FROM ".DB_THREADS." ft
		INNER JOIN ".DB_POSTS." fp ON ft.thread_id = fp.thread_id
