...
if (isset($_GET['step']) && $_GET['step'] == "renew") {
	$result = dbquery(
		"SELECT p.post_id, p.post_author, p.post_datestamp, p.post_alias FROM ".DB_POSTS." p
	...
	if (!isset($_POST['deletethread'])) {
		...
		if ($threads_count > 0) {
			...
			if (dbrows($result)) {
				$result = dbquery(
					"SELECT p.forum_id, p.post_author, p.post_datestamp, p.post_alias FROM ".DB_POSTS." p
					INNER JOIN ".DB_THREADS." t ON p.thread_id=t.thread_id
