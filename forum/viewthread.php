...
$forumorder1 = isset($_GET['order']) ? $_GET['order'] : (isset($userdata['user_forumorder']) ? $userdata['user_forumorder'] : 'ASC');
$forumorder1 = $forumorder1 == 'DESC' ? 'DESC' : 'ASC';
$forumorder2 = isset($_GET['order']) ? '&amp;order='.$forumorder1 : '';
...
if (dbrows($result)) {
	...
	//Forumkategorier - Array skal rettes!
	$forumcats = array(8 => 'Generel', 12 => 'Følelser', 16 => 'Samfund', 19 => 'Medier', 23 => 'Fritid', 28 => 'Muks', 40 => 'Internt');
	$fdata['forum_cat_name'] = isset($forumcats[$fdata['forum_cat']]) ? $forumcats[$fdata['forum_cat']] : '-';
...
if (isset($_GET['pid']) && isnum($_GET['pid'])) {
	$reply_count = dbcount("(post_id)", DB_POSTS, "thread_id='".$fdata['thread_id']."' AND post_id".($forumorder1 == 'ASC' ? '<=' : '>=')."'".$_GET['pid']."' AND post_hidden='0' AND post_firstpost = '0'");
...
list($rows, $last_post) = dbarraynum(dbquery(
	"SELECT COUNT(post_id), MAX(post_id) FROM ".DB_POSTS." WHERE thread_id='".$_GET['thread_id']."' AND post_hidden='0' AND post_firstpost = '0' GROUP BY thread_id"));
$rows++;
if (($rows > $posts_per_page) || ($can_post || $can_reply)) {
	echo "<table cellspacing='0' cellpadding='0' width='100%'>\n<tr>\n";
	if ($rows > $posts_per_page) { echo "<td style='padding:4px 0px 4px 0px'>".makepagenav($_GET['rowstart'],$posts_per_page,$rows,3,FUSION_SELF."?thread_id=".$_GET['thread_id'].$forumorder2."&amp;")."</td>\n"; }
	if (iMEMBER && $can_post) {
...
if ($rows != 0) {
	dbquery("UPDATE ".DB_THREADS." SET thread_postcount='$rows', thread_lastpostid='$last_post' WHERE thread_id='".$_GET['thread_id']."'");
	...
	$firstpost_result1 = dbquery(
		"SELECT p.post_replynum, p.post_alias, p.forum_id, p.thread_id, p.post_id, p.post_message, p.post_smileys, p.post_author,
		p.post_datestamp, p.post_edituser, p.post_edittime, p.post_editreason, p.post_hidden,
		u.user_id, u.user_name, u.user_status, u.user_avatar, u.user_level, u.user_groups, u.user_aliases, u.user_title
		FROM ".DB_POSTS." p
		LEFT JOIN ".DB_USERS." u ON p.post_author = u.user_id
		WHERE p.post_id='".$fdata['thread_firstpost']."' AND p.post_hidden='0'
	");

	$result = dbquery(
		"SELECT p.post_firstpost, p.post_replynum, p.post_alias, p.forum_id, p.thread_id, p.post_id, p.post_message, p.post_smileys, p.post_author,
		p.post_datestamp, p.post_edituser, p.post_edittime, p.post_editreason,
		u.user_id, u.user_name, u.user_status, u.user_avatar, u.user_level, u.user_groups, u.user_aliases, u.user_title
		FROM ".DB_POSTS." p
		LEFT JOIN ".DB_USERS." u ON p.post_author = u.user_id
		WHERE p.thread_id='".$_GET['thread_id']."' AND post_hidden='0' AND p.post_firstpost='0'
		ORDER BY p.post_datestamp ".$forumorder1." LIMIT ".$_GET['rowstart'].",$posts_per_page"
	);
	...
	while ($data = dbarray(!isset($firstpost_result1) ? $result : $firstpost_result1)) {

		unset($firstpost_result1);
		...
		if ($current_row == 1) {
			...
			if (iMEMBER && $settings['thread_notify']) {
				if (dbcount("(thread_id)", DB_THREAD_NOTIFY, "thread_id='".$_GET['thread_id']."' AND notify_user='".$userdata['user_id']."'")) {
					...
					echo "<a href='postify.php?post=off&amp;forum_id=".$fdata['forum_id']."&amp;thread_id=".$_GET['thread_id'].$forumorder2."'>".$locale['515']."</a>";
				} else {
					echo "<a href='postify.php?post=on&amp;forum_id=".$fdata['forum_id']."&amp;thread_id=".$_GET['thread_id'].$forumorder2."'>".$locale['516']."</a>";
				}
			}
		...
		echo "<tr>\n<td class='tbl2 forum_thread_user_name' style='width:140px'><!--forum_thread_user_name-->".alias2($data['post_alias'],$data['user_aliases'], $data['user_id'],  $data['user_name'], $data['user_status'])."</td>\n";
		echo "<td class='tbl2 forum_thread_post_date'>\n";
		echo "<div style='float:right' class='small'>";
		echo "&nbsp;<a href='#post_".$data['post_id']."' name='post_".$data['post_id']."' id='post_".$data['post_id']."'>#".$data['post_replynum']."</a>";
		echo "&nbsp;<a href='".BASEDIR."print.php?type=F&amp;thread=".$_GET['thread_id']."&amp;post=".$data['post_id']."&amp;nr=".$data['post_replynum']."'><img src='".get_image("printer")."' alt='".$locale['519a']."' title='".$locale['519a']."' style='border:0;vertical-align:middle' /></a></div>\n";
		...
		if (iMEMBER && ($can_post || $can_reply)) {
			if (!$fdata['thread_locked']) {
				echo "<a href='post.php?action=reply&amp;forum_id=".$data['forum_id']."&amp;thread_id=".$data['thread_id']."&amp;post_id=".$data['post_id']."&amp;quote=".$data['post_id']."'>Citér</a>\n";
				if (iMOD || (($lock_edit && $last_post['post_id'] == $data['post_id'] || !$lock_edit)) && ($userdata['user_id'] == $data['post_author']) && ($settings['forum_edit_timelimit'] <= 0 || time() - $settings['forum_edit_timelimit']*60 < $data['post_datestamp'])) {
					echo "- <a href='post.php?action=edit&amp;forum_id=".$data['forum_id']."&amp;thread_id=".$data['thread_id']."&amp;post_id=".$data['post_id']."'>Rediger</a>\n";
				}
			} elseif (iMOD) {
				echo "<a href='post.php?action=edit&amp;forum_id=".$data['forum_id']."&amp;thread_id=".$data['thread_id']."&amp;post_id=".$data['post_id']."'>Rediger</a>\n";
			}
		...
		if (iMEMBER)
		{
			echo ' - <a href="/report.php?action=new&amp;post_id='.$data['post_id'].'">Anmeld</a>';
		}
		echo "</div>\n</td>\n</tr>\n";
		if ($data['post_id'] == $fdata['thread_firstpost']) { echo "<tr>\n<td colspan='2' style='height: 30px; text-align: right;'><h3><a href='viewthread.php?thread_id=".$data['thread_id']."&amp;order=".($forumorder1 == 'ASC' ? 'DESC' : 'ASC')."'>".($forumorder1 == 'ASC' ? 'Nyeste øverst' : 'Ældste øverst')."</a></h3></td>\n</tr>"; }
		$current_row++;
	}
	/** javascript **/
	// highlight jQuery plugin
	if (isset($_GET['highlight'])) {
		$words = explode(" ", stripinput(urldecode($_GET['highlight'])));
...
if ($rows > $posts_per_page) {
	...
	echo makepagenav($_GET['rowstart'],$posts_per_page,$rows,3,FUSION_SELF."?thread_id=".$_GET['thread_id'].$forumorder2.(isset($_GET['highlight']) ? "&amp;highlight=".urlencode($_GET['highlight']):"")."&amp;")."\n";
	echo "</div>\n";
}
...
if ($can_reply && !$fdata['thread_locked']) {
	...
	echo '<select name="post_alias"><option value="-1">Brug ikke alias</option><option value="0">1: '.$userdata['user_aliases'][0].'</option><option value="1">2: '.$userdata['user_aliases'][1].'</option><option value="2">3: '.$userdata['user_aliases'][2].'</option></select>';
	echo "</td>\n";
	echo "</tr>\n<tr>\n";
