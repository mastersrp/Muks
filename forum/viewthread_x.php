...
if (isset($_GET['pid']) && isnum($_GET['pid'])) {
	$reply_count = dbcount("(post_id)", DB_POSTS, "thread_id='".$fdata['thread_id']."' AND post_id".($forumorder1 == 'ASC' ? '<=' : '>=')."'".$_GET['pid']."' AND post_firstpost = 0 AND post_hidden='0'");
...
$rowsx = $rows + 1;
if ($rowsx != 0) {
	dbquery("UPDATE ".DB_THREADS." SET thread_postcount='$rowsx' WHERE thread_id='".$_GET['thread_id']."'");
	...
	$result = dbquery(
		"SELECT p.forum_id, p.thread_id, p.post_id, p.post_message, p.post_smileys, p.post_author,
		p.post_datestamp, p.post_ip, p.post_ip_type, p.post_edituser, p.post_edittime, p.post_editreason, p.post_firstpost, p.post_hidden, p.post_replynum, p.post_alias,
		u.user_id, u.user_name, u.user_status, u.user_avatar, u.user_level, u.user_groups, u.user_aliases
		FROM ".DB_POSTS." p
		LEFT JOIN ".DB_USERS." u ON p.post_author = u.user_id
		WHERE p.thread_id='".$_GET['thread_id']."' AND p.post_hidden='0' AND p.post_firstpost < 1
		ORDER BY p.post_datestamp ".$forumorder1." LIMIT ".$_GET['rowstart'].", 20"
	);
...
/*
	$result = dbquery(
		...
		ORDER BY p.post_firstpost DESC, p.post_datestamp ".$forumorder1." LIMIT ".$_GET['rowstart'].", 21"
	);
*/
	$result3 = dbquery(
		"SELECT p.forum_id, p.thread_id, p.post_id, p.post_message, p.post_smileys, p.post_author,
	...
	while ($data = dbarray(!isset($result3) ? $result : $result3)) {
		unset($result3);
		...
		echo "&nbsp;<a href='#post_".$data['post_id']."' name='post_".$data['post_id']."' id='post_".$data['post_id']."'>#".($data['post_replynum'])."</a></div>\n";
...
//			echo "<a href='".BASEDIR."messages.php?msg_send=".$data['user_id']."'><img src='".get_image("pm")."' alt='".$locale['572']."' style='border:0;vertical-align:middle' /></a>\n";
		...
		if (iMEMBER && ($can_post || $can_reply)) {
			if (!$fdata['thread_locked']) {
				echo "<a href='post.php?action=reply&amp;forum_id=".$data['forum_id']."&amp;thread_id=".$data['thread_id']."&amp;post_id=".$data['post_id']."&amp;quote=".$data['post_id']."'><b>Citér</b></a>\n";
				if (iMOD || (($lock_edit && $last_post['post_id'] == $data['post_id'] || !$lock_edit)) && ($userdata['user_id'] == $data['post_author']) && ($settings['forum_edit_timelimit'] <= 0 || time() - $settings['forum_edit_timelimit']*60 < $data['post_datestamp'])) {
					echo "<a href='post.php?action=edit&amp;forum_id=".$data['forum_id']."&amp;thread_id=".$data['thread_id']."&amp;post_id=".$data['post_id']."'><b>Ret</b></a>\n";
				}
			} elseif (iMOD) {
				echo "<a href='post.php?action=edit&amp;forum_id=".$data['forum_id']."&amp;thread_id=".$data['thread_id']."&amp;post_id=".$data['post_id']."'><b>Ret</b></a>\n";
			}
...
if ($rows > $posts_per_page) {
	...
	echo makepagenav($_GET['rowstart'],$posts_per_page,$rows,3,FUSION_SELF."?thread_id=".$_GET['thread_id'].$forumorder.(isset($_GET['highlight']) ? "&amp;highlight=".urlencode($_GET['highlight']):"")."&amp;")."\n";
	echo "</div>\n";
}
