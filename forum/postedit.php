...
if (isset($_POST['savechanges'])) {
	if (isset($_POST['delete'])) {
		...
		if (dbrows($result)) {
			...
			{
				opentable('Fejl!');
				echo 'Du kan ikke slette førsteposten i et emne!<br />';
			...
			else
			{
				...
				if (dbrows($result)) {
					$result = dbquery("	SELECT p.forum_id, p.post_author, p.post_datestamp, p.post_alias
										FROM ".DB_POSTS." p
				...
				if ($posts) {
					...
					if (!empty($result)) {
						...
						$pdata2 = dbarray($result);
						$result = dbquery("UPDATE ".DB_THREADS." SET thread_lastpost='".$pdata2['post_datestamp']."', thread_lastpostid='".$pdata2['post_id']."', thread_postcount=thread_postcount-1, thread_lastuser='".$pdata2['post_author']."', thread_lastpost_alias=".$pdata2['post_alias']." WHERE thread_id='".$_GET['thread_id']."'");
					}
				}
